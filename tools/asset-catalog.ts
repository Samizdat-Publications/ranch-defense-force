...
function contactSheet(out: string, names: string[], scale: number, cols: number): void {
  ...
  cells.forEach((f, n) => {
    // Each cell reads from ITS OWN page: a contact sheet spans groups, and the
    // packer has no reason to have put them all on one image.
...
L.push('')
L.push(`${keys.length} frame keys across ${atlas.pages.length} pages in \`public/atlas-*.png\`.`)
L.push('')
