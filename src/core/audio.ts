/**
 * Audio (§6): sixteen effects and three cross-fading music layers.
 *
 * Effects come from two places, chosen per sound in `audio.json`. PHYSICAL
 * sounds — gunfire, impacts, stone, a bull scraping dirt — are generated by
 * ElevenLabs, which has a purpose-built sound-effects endpoint. ARCADE feedback
 * — an XP pickup, a level-up flourish, a UI blip — stays synthesised here from
 * oscillators and noise, because those are not real-world sounds and a
 * realistic recording of one fights the pixel art rather than serving it.
 *
 * **Synthesis is also the fallback for every sound, including the sampled
 * ones.** `play()` prefers a decoded sample and drops to the oscillator path
 * when the file is absent, so the game is fully audible with no API keys at
 * all and degrades one sound at a time rather than going silent.
 *
 * Music is real audio from Lyria, fetched offline by `npm run music`. Missing
 * music must never be an error in a game loop, so an absent file is remembered
 * and simply not played.
 *
...
export class Audio {
  ...
  /** Decoded effect samples. `null` means "tried, absent — use synthesis". */
  ...
  play(name: SfxName): void {
    ...
    // Prefer a generated sample when one has been loaded. Its envelope is in
    // the audio already, so it plays flat rather than through the synth's decay.
    const sample = this.sfxBuffers.get(name)
    if (sample) {
      // Samples come back near full scale; the oscillator path peaks far lower
      // for the same `gain`. Scaling here keeps a sampled effect level with its
      // synthesised neighbour without retuning sixteen hand-set gains.
      const scale = (AUDIO.master as { sampleGainScale?: number }).sampleGainScale ?? 0.5
      gain.gain.setValueAtTime(spec.gain * scale, now)
    ...
    if (sample === undefined) void this.loadSfx(name)
  ...
  /**
   * Fetch and decode one effect sample, once. Marked `null` on failure so a
   * missing file is never retried — this is called from the hot path.
   */
  private async loadSfx(name: string): Promise<void> {
    if (this.sfxBuffers.has(name) || !this.ctx) return
    const spec = (AUDIO.sfx as Record<string, { source?: string; file?: string }>)[name]
    if (!spec?.file || spec.source !== 'elevenlabs') {
      this.sfxBuffers.set(name, null)
      return
    }
    // Claim the slot immediately: play() fires many times a second and would
    // otherwise start a dozen parallel fetches for the same file.
    this.sfxBuffers.set(name, null)
    try {
      const res = await fetch(`${import.meta.env.BASE_URL}audio/${spec.file}`)
      if (!res.ok) return
      const buffer = await this.ctx.decodeAudioData(await res.arrayBuffer())
      this.sfxBuffers.set(name, buffer)
    } catch {
      // Stays null; synthesis carries this sound.
    }
  }

  /** Warm the sampled effects so the first shot is not the synth one. */
  preload(): void {
    if (!this.ctx) return
    for (const [name, spec] of Object.entries(
      AUDIO.sfx as Record<string, { source?: string }>,
    )) {
      if (spec.source === 'elevenlabs') void this.loadSfx(name)
    }
  }
